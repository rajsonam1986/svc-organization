...
@Component
public class BranchCreateValidator {
    @Autowired
    ...
    public void validateBranchId(Long branchId) {
        ...
        if (branchId != null) {
            throw new CustomParameterizedException("A new branch cannot already have an ID", branchId);
        }
    }
    ...
    public void validateBranchIsAvailable(String branchName, Long courseId) {

        if (branchRepository.findByBranchNameAndCourse(branchName, courseRepository.findById(courseId).get()).isPresent()) {
    ...
    public void validateBranch(BranchDTO branchDTO) {
        ...
        validateBranchIsAvailable(branchDTO.getBranchName(), branchDTO.getCourseId());
