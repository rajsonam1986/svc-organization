...
@Component
public class SubjectUpdateValidator {
    @Autowired
    ...
    public void validateBranchIsAvailable(String subjectName, Long branchId, Long subjectId) {

        if (!subjectService.findByBranchIdAndSubjectId(branchId, subjectId).getSubjectName().equalsIgnoreCase(subjectName.trim())) {
            ...
            if (subjectRepository.findBySubjectNameAndBranch(subjectName, modelMapper.map(branchService.findOne(branchId), Branch.class)).isPresent()) {
                throw new CustomParameterizedException(ErrorConstantsOrganization.SUBJECT_EXISTS, subjectName);
            }
        }
    }
    ...
    public void validateSubject(SubjectDTO subjectDTO) {
        ...
        validateBranchIsAvailable(subjectDTO.getSubjectName(), subjectDTO.getBranchId(), subjectDTO.getSubjectId());
